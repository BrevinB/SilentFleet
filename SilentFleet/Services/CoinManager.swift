...
enum CoinManager {
    private static let winAmount = 40
    private static let perShipSunkOnWin = 5
    private static let perShipSunkOnLoss = 2
    private static let completionAmount = 10

    /// Bonus coins per win-streak level (streak 2 = 12, 3 = 25, 4 = 37, 5+ = 50).
    static func streakBonus(for streak: Int) -> Int {
        guard streak >= 2 else { return 0 }
        return min(streak - 1, 4) * 12
    }
    ...
    static func calculateReward(for state: GameState, streak: Int = 0) -> CoinReward {
        ...
        let perSink = playerWon ? perShipSunkOnWin : perShipSunkOnLoss

        return CoinReward(
            winBonus: playerWon ? winAmount : 0,
            sinkBonus: shipsSunk * perSink,
